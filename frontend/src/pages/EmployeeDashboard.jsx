...
import { Briefcase, LogOut, UserPlus, Car, AlertTriangle, ArrowLeft, RefreshCw, Trash2, CheckCircle, XCircle } from 'lucide-react';
...
const EmployeeDashboard = () => {
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
                ...
                <motion.button
                    ...
                    className="glass p-8 rounded-3xl text-left group border border-white/40 hover:border-white/60 transition-all shadow-lg hover:shadow-xl"
                >
                    ...
                    <div className="flex items-center text-indigo-600 font-semibold text-sm mt-4 group-hover:gap-2 transition-all">
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/30 to-slate-50 relative overflow-hidden">
            ...
            {/* Navbar */}
            <nav className="glass sticky top-0 z-50 px-6 py-4 border-b border-white/20 backdrop-blur-xl">
                <div className="max-w-6xl mx-auto flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <button
                            ...
                            className="p-2.5 hover:bg-red-50 rounded-xl text-slate-500 hover:text-red-500 transition-colors"
                            title="Logout"
                        >
            ...
            <main className="max-w-6xl mx-auto p-6 relative z-10">
                ...
                {activeModule === 'drivers' && (
                    <motion.div
                        ...
                        className="glass p-8 rounded-3xl max-w-2xl mx-auto shadow-xl"
                    >
                        <div className="flex items-center gap-3 mb-6">
                            <div className="bg-gradient-to-br from-blue-500 to-cyan-500 p-3 rounded-xl">
                                <UserPlus className="w-6 h-6 text-white" />
                            </div>
                            <h2 className="text-2xl font-bold text-slate-800">Register New Driver</h2>
                        </div>
                        <form onSubmit={handleRegisterDriver} className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1.5">First Name</label>
                                    <input type="text" placeholder="John" required className="input" value={driverForm.firstName} onChange={e => setDriverForm({ ...driverForm, firstName: e.target.value })} />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1.5">Last Name</label>
                                    <input type="text" placeholder="Doe" required className="input" value={driverForm.lastName} onChange={e => setDriverForm({ ...driverForm, lastName: e.target.value })} />
                                </div>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1.5">Email</label>
                                <input type="email" placeholder="john.doe@example.com" required className="input" value={driverForm.email} onChange={e => setDriverForm({ ...driverForm, email: e.target.value })} />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1.5">Password</label>
                                <input type="password" placeholder="••••••••" required className="input" value={driverForm.password} onChange={e => setDriverForm({ ...driverForm, password: e.target.value })} />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1.5">Phone</label>
                                <input type="text" placeholder="+20 123 456 7890" required className="input" value={driverForm.phone} onChange={e => setDriverForm({ ...driverForm, phone: e.target.value })} />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1.5">License Number</label>
                                <input type="text" placeholder="ABC123456" required className="input" value={driverForm.licenseNumber} onChange={e => setDriverForm({ ...driverForm, licenseNumber: e.target.value })} />
                            </div>
                    ...
                    <div className="space-y-6">
                        <div className="grid md:grid-cols-2 gap-6">
                            ...
                            >
                                ...
                                <form onSubmit={handleRegisterCar} className="space-y-4">
                                    <div>
                                        ...
                                        <input type="text" placeholder="Toyota Corolla" required className="input" value={carForm.model} onChange={e => setCarForm({ ...carForm, model: e.target.value })} />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 mb-1.5">License Plate</label>
                                        <input type="text" placeholder="ABC 1234" required className="input" value={carForm.licensePlate} onChange={e => setCarForm({ ...carForm, licensePlate: e.target.value })} />
                                    </div>
                                    <div>
                            ...
                            >
                                ...
                                <form onSubmit={handleAssignCar} className="space-y-4">
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 mb-1.5">Select Car</label>
                                        <select required className="input" value={assignForm.carId} onChange={e => setAssignForm({ ...assignForm, carId: e.target.value })}>
                                        ...
                                        <p className="text-xs text-slate-500 mt-1">{availableCars.length} available cars</p>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 mb-1.5">Select Driver</label>
                                        <select required className="input" value={assignForm.driverId} onChange={e => setAssignForm({ ...assignForm, driverId: e.target.value })}>
                                            <option value="">Choose a driver...</option>
                                            {driversWithoutCar.map(d => (
                                                <option key={d.id} value={d.id}>
                                                    {d.firstName} {d.lastName} - {d.email}
                                                </option>
                                            ))}
                                        </select>
                                        <p className="text-xs text-slate-500 mt-1">{driversWithoutCar.length} drivers without cars</p>
                                    </div>
                                    <button type="submit" disabled={loading} className="btn btn-primary w-full">
                        ...
                        >
                            <div className="flex justify-between items-center mb-6">
                                <h3 className="text-lg font-bold text-slate-800">All Cars</h3>
                                <button onClick={fetchCars} disabled={loading} className="p-2 hover:bg-indigo-50 rounded-xl transition-colors">
                                    <RefreshCw className={`w-5 h-5 text-indigo-600 ${loading ? 'animate-spin' : ''}`} />
                                </button>
                            </div>
                            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {cars.length === 0 ? (
                                    <p className="text-slate-400 col-span-full text-center py-8">No cars registered</p>
                                ) : (
                                    cars.map(car => (
                                        <div key={car.id} className="p-4 border-2 border-slate-100 rounded-2xl bg-white hover:shadow-md transition-all hover:border-indigo-100">
                                            <div className="flex justify-between items-start mb-3">
                                                <div>
                                                    <span className="font-bold text-slate-800 text-lg">{car.model}</span>
                                                    <p className="text-sm text-slate-500 mt-0.5">{car.licensePlate}</p>
                                                </div>
                                                <button onClick={() => handleDeleteCar(car.id)} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                                                    <Trash2 className="w-4 h-4" />
                                                </button>
                                            </div>
                                            <div className="flex items-center justify-between">
                                                <span className="text-sm text-slate-600">🎨 {car.color}</span>
                                                <span className={`px-2 py-1 text-xs rounded-full font-semibold ${car.driver ? 'bg-slate-100 text-slate-600' : 'bg-emerald-100 text-emerald-600'}`}>
                                                    {car.driver ? '🔒 Assigned' : '✅ Available'}
                                                </span>
                                            </div>
                                            {car.driver && (
                                                <p className="text-xs text-indigo-600 mt-2 font-medium bg-indigo-50 px-2 py-1 rounded-lg">
                    ...
                    <div className="space-y-6">
                        {['new', 'open', 'closed'].map(status => (
                            <motion.div
                                ...
                                className="glass p-6 rounded-3xl shadow-lg"
                            >
                                <h3 className="text-lg font-bold text-slate-800 mb-4 uppercase flex items-center gap-2">
                                ...
                                <div className="space-y-3">
                                    {complaints[status].length === 0 ? (
                                        ...
                                        complaints[status].map(c => (
                                            <div key={c.id} className="p-4 border-2 border-slate-100 rounded-2xl bg-white hover:border-indigo-100 transition-colors">
                                                <div className="flex justify-between items-start gap-4">
                                                    <div className="flex-1">
                                                        <p className="font-medium text-slate-800 mb-1">{c.message}</p>
                                                    ...
                                                    <div className="flex gap-2">
                                                        {status === 'new' && (
                                                            <button
                                                                ...
                                                                disabled={loading}
                                                                className="px-4 py-2 bg-blue-100 text-blue-600 rounded-xl text-sm font-semibold hover:bg-blue-200 transition-colors flex items-center gap-1"
                                                            >
                                                        ...
                                                        {status === 'open' && (
                                                            <button
                                                                ...
                                                                disabled={loading}
                                                                className="px-4 py-2 bg-emerald-100 text-emerald-600 rounded-xl text-sm font-semibold hover:bg-emerald-200 transition-colors flex items-center gap-1"
                                                            >
